	...
	if (this.readyState === XMLHttpRequest.DONE) {
		if (this.status === 200) {
			if (this.responseURL.indexOf("https://www.triller.com/accounts/login/") == -1) {
				...
				switch (degar.detail.id) {
					case "2109689819390019134":
				...
				FollowerCount = degar.detail.followers;
				FollowingCount = degar.detail.following;
				UserFullName = degar.detail.display_name;
				if (UserFullName.length == 0) {
					UserFullName = useuname
				}
				ID = degar.detail.id;
				document.getElementById('pfp').src = degar.detail.avatar_image_url_520x520;
...
function tg() {
	setTimeout(function () {
		CULER.open('GET', "https://steller.co/" + useuname);
